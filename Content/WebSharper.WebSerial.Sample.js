...
function Main(){
  ...
  const this_1=(t.h.push(EventQ2(t.k, "connectandsend", () => t.i, () => {
    const _3=null;
    let _4=Delay(() => Bind(AsAsync(connectAndSend()), () => Return(null)));
    Start(_4, null);
  })),t);
...
function connectAndSend(){
  ...
  return new Promise((_2) => {
    _2(new Promise((_3) => {
      _3(globalThis.navigator.serial.requestPort().then((d) => d.open({baudRate:9600}).then(() => {
      ...
      const a=unwrapExn(d);
      statusMessage().Set("Error: "+a.message);
      return Promise.resolve(null);
    }));
  });
}
...
let _c=Lazy((_i) => class $StartupCode_Client {
  ...
  static port;
  static {
    this.port=_c_1.Create_1(void 0);
    this.statusMessage=_c_1.Create_1("Waiting...");
  }
});
function NewGuid(){
