...
function Main(){
  ...
  const this_1=(t.h.push(EventQ2(t.k, "connectandsend", () => t.i, () => {
    const _3=null;
    Start(Delay(() => Bind(AsAsync(connectAndSend()), () => Return(null))), null);
  })),t);
  const b=(this_1.h.push(new _c_6("status", statusMessage().View)),this_1);
...
function connectAndSend(){
  ...
  return new Promise((_2) => {
    _2(new Promise((_3) => {
      _3(serial().requestPort().then((d) => d.open({baudRate:9600}).then(() => {
        const writer=d.writable.getWriter();
        return OfAsync(AsAsync(writer.write(eval("new TextEncoder()").encode("Hello\n")))).then(() => {
          writer.releaseLock();
          statusMessage().Set("Message Sent!");
...
let _c_7=Lazy((_i) => class $StartupCode_Client {
  ...
  static port;
  static {
    this.port=_c_12.Create_1(void 0);
    this.serial=globalThis.navigator.serial;
    this.statusMessage=_c_12.Create_1("Waiting...");
  }
});
function NewGuid(){
